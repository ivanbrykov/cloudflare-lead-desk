...
2. Create the D1 database in your account, with any name you like (it does not need to match the Worker): `pnpm exec wrangler d1 create lead-desk-db`. Copy the `database_id` it prints into the `d1_databases` entry in `wrangler.jsonc` (replacing the empty string) and set `database_name` to the name you chose.
