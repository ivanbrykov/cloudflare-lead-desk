...
-- Renumber stages within each pipeline into a deterministic sequence ordered
-- by (created_at, id), so duplicate (pipeline_id, position) rows — which were
-- possible before the unique index existed — are resolved with a stable,
-- idempotent order before uniqueness is enforced. Rows already in creation
-- order keep their positions.
UPDATE stages SET position = (SELECT COUNT(*) - 1 FROM stages AS earlier WHERE earlier.pipeline_id = stages.pipeline_id AND (earlier.created_at < stages.created_at OR (earlier.created_at = stages.created_at AND earlier.id <= stages.id)));--> statement-breakpoint
